...
function getComposition(f, g) {
  return function getResult(x) { return f(g(x)); };
}
...
function getPowerFunction(exponent) {
  return function exponentiate(basis) { return basis ** exponent; };
}
...
function memoize(func) {
  ...
  return function getMemoizeResult() {
    if (storage !== undefined) {
      return storage;
    }

    storage = func.apply(this);
    return storage;
  };
}
...
function retry(func, attempts) {
  return function getRetryResult() {
    function useAttempt(attempt) {
      if (attempt <= 0) {
        return undefined;
      }
      ...
      try {
        ...
        return useAttempt(attempt - 1);
      }
    }

    return useAttempt(attempts);
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function getPartialUsingArgumentsFunc(...args2) {
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function getNextIdentifier() {
