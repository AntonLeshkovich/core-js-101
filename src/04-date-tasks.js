...
function timeSpanToString(startDate, endDate) {
  ...
  const milliseconds = Math.floor(difference % 1000).toString().padStart(3, '0');
...
function angleBetweenClockHands(date) {
  ...
  const hoursAng = (formattedHours * 30 + minutes * 0.5) % 360;
  const minutesAng = (minutes * 6) % 360;

  let angle = Math.abs(hoursAng - minutesAng);
  if (angle > 180) {
    angle = 360 - angle;
  }
